...
if not os.path.exists(csv_filename):
    with open(csv_filename, 'w', newline='') as f:
        csv.writer(f).writerow([
            "Round", "Train Accuracy", "Train Loss", "Test Accuracy", "Test Loss",
            "Val Accuracy", "Val Loss", "Round Training Time", "Checkpoint Times",
            "Inference Time", "Total Training Time", "Detection Accuracy",
            "Detected Malicious Clients", "Ground Truth Malicious Clients"
        ])
...
for round in range(start_round, args.num_rounds):
    ...
    if args.aggregation == "fedavg":
        ...
        # Create ground truth labels for evaluation: 0 = benign, 1 = malicious
        ground_truth = [1 if i in attacker_clients else 0 for i in range(args.n_clients)]
        aggregated_update, detection_accuracy, detected_malicious_clients = bartfl(
            local_updates, device, num_clusters=args.num_clusters, ground_truth=ground_truth, pca_dim=1.0
        )
    else:
        aggregated_update = agg_method(local_updates, device, f=args.n_attackers)
    # Set default values when not BART-FL
    if args.aggregation != "bartfl":
        detection_accuracy = "NA"
        detected_malicious_clients = "NA"
    ...
    with open(csv_filename, 'a', newline='') as f:
        with open(csv_filename, 'a', newline='') as f:
            csv.writer(f).writerow([
                ...
                detection_accuracy,  # detection accuracy from BART-FL
                str(detected_malicious_clients),  # detected malicious clients
                str(attacker_clients)  # ground truth (actual attackers)
            ])
